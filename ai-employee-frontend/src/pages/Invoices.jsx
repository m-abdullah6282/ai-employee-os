    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-400">{invoices.length} total invoices</p>
        </div>
        <button
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-700">
              <thead className="bg-gray-800">
                <tr>
                  {['Number', 'Contact', 'Issue Date', 'Due Date', 'Total', 'Status'].map((h) => (
                    <th
                      key={h}
              ...
              <tbody className="divide-y divide-gray-700">
                {invoices.map((inv) => (
                  <tr key={inv.id} className="transition-colors hover:bg-gray-700/50">
                    <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-white">{inv.invoice_number}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-300">{getContactName(inv.contact_id)}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-300">{inv.issue_date}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-300">{inv.due_date}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm font-semibold text-white">
                      {inv.currency} {Number(inv.total).toFixed(2)}
                    </td>
                    <td className="whitespace-nowrap px-6 py-4">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-medium capitalize ${STATUS_STYLES[inv.status] || STATUS_STYLES.draft}`}>
                        {inv.status}
                      </span>
                    </td>
                  </tr>
        ...
        <form onSubmit={handleCreate} className="space-y-5">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              {contacts.length === 0 && (
                <p className="mt-1 text-xs text-yellow-500">Create a contact first to create invoices.</p>
              )}
            </div>
            <div>
            ...
            <div>
              ...
              <input required type="date" name="issue_date" value={form.issue_date} onChange={handleChange} className={inputClass} />
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-gray-300">Due Date *</label>
              <input required type="date" name="due_date" value={form.due_date} onChange={handleChange} className={inputClass} />
            </div>
            <div>
